...
void UARTInit( uint32_t PortNum, uint32_t baudrate )
{
  ...
  if ( PortNum == 0 )
  {
    ...
    LPC_PINCON->PINSEL0 |= (1 << 6);             /* Pin P0.3 -- RXD0 (Com0) */
            LPC_UART0->LCR  = 0x83;                       /* 8 bits, 无校验, 1 个停止位 */
    Fdiv = (SystemCoreClock/4/ 16) / baudrate;/* 设置波特率 */
    LPC_UART0->DLM  = Fdiv / 256;
    LPC_UART0->DLL  = Fdiv % 256+1;
   LPC_UART0->LCR  = 0x03;                       /* 锁定波特率  */
    LPC_UART0->FCR  = 0x07;                       /* 串口先进先出控制寄存器设置*/
  ...
  else if ( PortNum == 1 )
  {
          ...
          LPC_PINCON->PINSEL4 |= (1<<3);                /* Pin P2.1 -- RXD1 (Com1)*/
               ...
               LPC_UART1->LCR = 0x83;                                  /* 8 bits, 无校验, 1 个停止位 */
    Fdiv = (SystemCoreClock/4/16)/ baudrate ; /* 设置波特, Fpclk: 18MHz */
    LPC_UART1->DLM = Fdiv/256;                                                                                             
    LPC_UART1->DLL = (Fdiv%256)+1;
               LPC_UART1->LCR = 0x03;                                   /* DLAB = 0 */
    LPC_UART1->FCR = 0x07;                             /* 使能TX与 RX 并初始化FIFO. */
...
** Descriptions:        从串口获取单个字符，目的是和stdio连接
** input parameters:    无
** output parameters:   无
** Returned value:      获取的字符
*********************************************************************************************************/
int getkey(void)
{
